...
namespace Membera.Auth.Application.Tests.Auth.Admin.DeleteAdminAccount;

public class DeleteAdminAccountHandlerTests
{
    private readonly Mock<IUserRepository> _userRepositoryMock;
    private readonly Mock<IRefreshTokenRepository> _refreshTokenRepositoryMock;
    private readonly Mock<ILogger<DeleteAdminAccountHandler>> _loggerMock;
    private readonly DeleteAdminAccountHandler _handler;

    public DeleteAdminAccountHandlerTests()
    {
        _userRepositoryMock = new Mock<IUserRepository>();
        _refreshTokenRepositoryMock = new Mock<IRefreshTokenRepository>();
        _loggerMock = new Mock<ILogger<DeleteAdminAccountHandler>>();

        _handler = new DeleteAdminAccountHandler(
    ...
    public async Task HandleAsync_WhenTargetIsRequestingUser_ThrowsInvalidOperationException()
    {
        ...
        var command = new DeleteAdminAccountCommand(sharedId, sharedId);
    ...
    {
        // Arrange
        var command = new DeleteAdminAccountCommand(Guid.NewGuid(), Guid.NewGuid());

        _userRepositoryMock
            .Setup(r => r.GetByIdAsync(command.TargetAdminId))
    ...
    [Fact]
    public async Task HandleAsync_WhenAdminAlreadyDeleted_ThrowsInvalidOperationException()
    {
        // Arrange
        var command = new DeleteAdminAccountCommand(Guid.NewGuid(), Guid.NewGuid());

        var deletedAdmin = new User("Admin", "User", "admin@test.com", "hash-in-db", UserRole.Admin);
        deletedAdmin.MarkAsDeleted();

        _userRepositoryMock
            .Setup(r => r.GetByIdAsync(command.TargetAdminId))
            .ReturnsAsync(deletedAdmin);
    ...
    [Fact]
    public async Task HandleAsync_WithAdminTarget_MarksAdminDeletedAndRevokesAllRefreshTokens()
    {
        // Arrange
        var command = new DeleteAdminAccountCommand(Guid.NewGuid(), Guid.NewGuid());
        ...
        // Assert
        Assert.True(adminUser.IsDeleted);
        Assert.NotNull(adminUser.DeletedAt);
